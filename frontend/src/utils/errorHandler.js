// Аналізує помилку від API та повертає текст українською мовою
export const getErrorMessage = (error) => {
  // Помилка мережі або сервер недоступний
  if (!error.response) {
  ...
  // Обробка специфічних бізнес-помилок нашого бекенду (з exceptions.py)
  if (data && data.code) {
    switch (data.code) {
      case 'CR_TOO_HIGH':
  ...
  // Обробка стандартних HTTP-статусів
  switch (status) {
    case 400:
