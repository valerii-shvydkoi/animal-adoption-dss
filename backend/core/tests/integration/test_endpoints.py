...
class TestIntegrationEndpoints:
    ...
    def test_questionnaire_invalid_cr(self, mock_is_valid, auth_client):
        ...
        response = auth_client.post('/api/v1/questionnaire/', {"matrix": {}}, format='json')
    ...
    def test_pet_deletion_cascades_to_adoptions(self, auth_volunteer_client, pet, adoption_request):
    ...
    def test_duplicate_volunteer_request(self, mock_create, auth_client, shelter):
        ...
        response = auth_client.post('/api/v1/volunteer/requests/', {"shelter": shelter.id})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_login_throttling_limit(self, api_client, user):
        ...
        response = api_client.post(url, {"email": user.email, "password": "wrong_password"})
