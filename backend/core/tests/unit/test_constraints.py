...
class ConstraintsTest(TestCase):
    def setUp(self):
        ...
        self.heavy_pet = Pet.objects.create(
            name="Великий пес", shelter=self.shelter, weight=25.0,
            activity_level=3, sociability=3, stress_resistance=3
        )
        self.light_pet = Pet.objects.create(
            name="Малий пес", shelter=self.shelter, weight=5.0,
            activity_level=3, sociability=3, stress_resistance=3
        )
        self.stressed_pet = Pet.objects.create(
            name="Стресовий пес", shelter=self.shelter, weight=10.0,
            activity_level=3, sociability=3, stress_resistance=1
        )
