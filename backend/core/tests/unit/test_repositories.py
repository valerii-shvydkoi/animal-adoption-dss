...
class PetRepositoryTest(TestCase):
    def setUp(self):
        ...
        self.available_pet = Pet.objects.create(
            name="Доступний", shelter=self.shelter, is_available=True,
            activity_level=3, sociability=3, stress_resistance=3, weight=10.0
        )
        self.unavailable_pet = Pet.objects.create(
            name="Зайнятий", shelter=self.shelter, is_available=False,
            activity_level=3, sociability=3, stress_resistance=3, weight=10.0
        )
