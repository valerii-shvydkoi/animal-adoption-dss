# Матриця вимог Adoptify

Документ звіряє реалізацію Adoptify з функціональними сценаріями UC1-UC17.

| UC   | Вимога                                               | Реалізація в проєкті                                                                        | Стан        |
| ---- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------- | ----------- |
...
| UC6  | Подання заявки на адаптацію                           | `frontend/src/pages/CreateRequest.jsx`, `backend/core/services/adoption_service.py`         | Реалізовано |
...
- Активними заявками на адаптацію є `PENDING`, `REVIEWED`, `APPROVED`; `REJECTED` і `CANCELLED` не блокують повторну подачу.
...
- Гостьове обране переноситься в акаунт після входу та одразу очищується з гостьового сховища; серверна синхронізація не дублює гостьову пам'ять.
...
- Throttling: `anon=30/min`, `user=120/min`, `login=10/min`.
...
- Транзакційність: заявки на адаптацію створюються та схвалюються через `transaction.atomic()`.
